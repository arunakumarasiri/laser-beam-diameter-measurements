...
/** @cond DO_NOT_WANT */
#define LIBXIMC_VERSION 2.9.11
/** @endcond */


/** @cond DO_NOT_WANT */
#define LIBXIMC_PROTOCOL_VERSION 17.3
/** @endcond */
...
#define STATE_IS_HOMED                 0x00020 	/**< \english Calibration performed \endenglish \russian Калибровка выполнена \endrussian */
...
#define STATE_CURRENT_MOTOR_BITS       0xC0000 	/**< \english Bits indicating the current operating motor on boards with multiple outputs for engine mounting. \endenglish \russian Биты, показывающие текущий рабочий мотор на платах с несколькими выходами для двигателей. \endrussian */
#define STATE_CURRENT_MOTOR0           0x00000 	/**< \english Motor 0. \endenglish \russian Мотор 0. \endrussian */
#define STATE_CURRENT_MOTOR1           0x40000 	/**< \english Motor 1. \endenglish \russian Мотор 1. \endrussian */
#define STATE_CURRENT_MOTOR2           0x80000 	/**< \english Motor 2. \endenglish \russian Мотор 2. \endrussian */
#define STATE_CURRENT_MOTOR3           0xC0000 	/**< \english Motor 3. \endenglish \russian Мотор 3. \endrussian */
//@}
...
#define ENGINE_CURRENT_AS_RMS   0x02 	/**< \english Engine current meaning flag. If the flag is set, then engine current value is interpreted as root mean square current value. If the flag is unset, then engine current value is interpreted as maximum amplitude value. \endenglish \russian Флаг интерпретации значения тока. Если флаг установлен, то задаваемое значение тока интерпретируется как среднеквадратичное значение тока, если флаг снят, то задаваемое значение тока интерпретируется как максимальная амплитуда тока. \endrussian */
...
#define FEEDBACK_ENC_TYPE_AUTO           0x00 	/**< \english Auto detect encoder type. \endenglish \russian Определять тип энкодера автоматически. \endrussian */
...
/** 
	...
	*/
	typedef struct
	{
		unsigned int FastHome;	/**< \english Speed used for first motion. Range: 0..100000. \endenglish \russian Скорость первого движения. Диапазон: 0..100000 \endrussian */
		unsigned int uFastHome;	/**< \english Part of the speed for first motion, microsteps. \endenglish \russian Дробная часть скорости первого движения в микрошагах(используется только с шаговым двигателем). \endrussian */
		unsigned int SlowHome;	/**< \english Speed used for second motion. Range: 0..100000. \endenglish \russian Скорость второго движения. Диапазон: 0..100000. \endrussian */
		unsigned int uSlowHome;	/**< \english Part of the speed for second motion, microsteps. \endenglish \russian Дробная часть скорости второго движения в микрошагах(используется только с шаговым двигателем). \endrussian */
		int HomeDelta;	/**< \english Distance from break point. \endenglish \russian Расстояние отхода от точки останова. \endrussian */
...
/** 
	...
	*/
	typedef struct
	{
		...
		unsigned int uSpeed;	/**< \english Target speed in microstep fractions/s. Using with stepper motor only. \endenglish \russian Заданная скорость в единицах деления микрошага в секунду. Используется только с шаговым мотором. \endrussian */
		...
		unsigned int uAntiplaySpeed;	/**< \english Speed in antiplay mode, 1/256 microsteps/s. Used with stepper motor only. \endenglish \russian Скорость в режиме антилюфта, выраженная в 1/256 микрошагах в секунду. Используется только с шаговым мотором. \endrussian */
	} move_settings_t;
...
/** 
	...
	*/
	typedef struct
	{
		...
		unsigned int NomSpeed;	/**< \english Nominal speed (in whole steps/s or rpm for DC and stepper motor as a master encoder). Controller will keep motor shaft RPM below this value if ENGINE_LIMIT_RPM flag is set. Range: 1..100000. \endenglish \russian Номинальная скорость (в целых шагах/с или rpm для DC и шагового двигателя в режиме ведущего энкодера). Контроллер будет сохранять скорость мотора не выше номинальной, если установлен флаг ENGINE_LIMIT_RPM. Диапазон: 1..100000. \endrussian */
		unsigned int uNomSpeed;	/**< \english The fractional part of a nominal speed in microsteps (is only used with stepper motor). \endenglish \russian Микрошаговая часть номинальной скорости мотора (используется только с шаговым двигателем). \endrussian */
...
/**  
	...
	*/
	typedef struct
	{
		...
		int uRightBorder;	/**< \english Right border position in 1/256 microsteps. Used with stepper motor only. Range: -255..255. \endenglish \russian Позиция правой границы в 1/256 микрошагах (используется только с шаговым двигателем). Диапазон: -255..255. \endrussian */
	} edges_settings_t;
...
/** 
	...
	*/
	typedef struct
	{
		...
		int uPosition;	/**< \english The fractional part of a position or shift in microsteps. Is used with stepper motor. Range: -255..255. \endenglish \russian Дробная часть позиции или смещения в микрошагах. Используется только с шаговым двигателем. Диапазон: -255..255. \endrussian */
		unsigned int Speed;	/**< \english Target speed (for stepper motor: steps/s, for DC: rpm). Range: 0..100000. \endenglish \russian Заданная скорость (для ШД: шагов/c, для DC: rpm). Диапазон: 0..100000. \endrussian */
		unsigned int uSpeed;	/**< \english Target speed in microsteps/s. Using with stepper motor only. \endenglish \russian Заданная скорость в микрошагах в секунду. Используется только с шаговым мотором. \endrussian */
	} sync_in_settings_t;
...
/** 
	...
	*/
	typedef struct
	{
		...
		unsigned int SyncOutPeriod;	/**< \english This value specifies number of encoder pulses or steps between two output synchronization pulses when SYNCOUT_ONPERIOD is set. \endenglish \russian Период генерации импульсов, используется при установленном флаге SYNCOUT_ONPERIOD. \endrussian */
		...
		unsigned int uAccuracy;	/**< \english This is the neighborhood around the target coordinates in micro steps (only used with stepper motor). \endenglish \russian Это окрестность вокруг целевой координаты в микрошагах (используется только с шаговым двигателем). \endrussian */
	} sync_out_settings_t;
...
/** 
	...
	*/
	typedef struct
	{
		...
		unsigned int uMaxSpeed[10];	/**< \english Array of speeds (1/256 microstep) using with joystick and button control. \endenglish \russian Массив скоростей (в 1/256 микрошагах), использующийся при управлении джойстиком или кнопками влево/вправо. \endrussian */
		...
		int uDeltaPosition;	/**< \english Fractional part of the shift in micro steps. Is only used with stepper motor. Range: -255..255. \endenglish \russian Дробная часть смещения в микрошагах. Используется только с шаговым двигателем. Диапазон: -255..255. \endrussian */
	} control_settings_t;
...
/** 
	...
	* \endrussian
	* @see command_add_sync_in_action
	*/
	typedef struct
	{
		int Position;	/**< \english Desired position or shift (whole steps) \endenglish \russian Желаемая позиция или смещение (целая часть) \endrussian */
		int uPosition;	/**< \english The fractional part of a position or shift in microsteps. Is only used with stepper motor. Range: -255..255. \endenglish \russian Дробная часть позиции или смещения в микрошагах. Используется только с шаговым двигателем. Диапазон: -255..255. \endrussian */
		unsigned int Time;	/**< \english Time for which you want to achieve the desired position in microseconds. \endenglish \russian Время, за которое требуется достичь требуемой позиции, в микросекундах. \endrussian */
	} command_add_sync_in_action_t;

	typedef struct
	{
		float Position;	/**< \english Desired position or shift. \endenglish \russian Желаемая позиция или смещение. \endrussian */
		unsigned int Time;	/**< \english Time for which you want to achieve the desired position in microseconds. \endenglish \russian Время, за которое требуется достичь требуемой позиции, в микросекундах. \endrussian */
	} command_add_sync_in_action_calb_t;
...
/** 
        ...
        * \endrussian
	* @see command_change_motor
        */
	typedef struct
	{
		unsigned int Motor;	/**< \english Motor number which it should be switch relay on [0..1] \endenglish \russian Номер мотора, на который следует переключить реле [0..1] \endrussian */
	} command_change_motor_t;
...
/** 
	...
	* Функция используется только производителем.
	* @param id идентификатор устройства
	* @param[in] serial number структура, содержащая серийный номер, версию железа и ключ.
	* \endrussian
	*/
...
/** 
	...
	* @param id an identifier of device
	* @param[out] random sequence generated by the controller
	* \endenglish
	* \russian
	* Чтение случайного числа из контроллера.
	* @param id идентификатор устройства
	* @param[out] случайная последовательность, сгенерированная контроллером
	* \endrussian
	*/
...
/**
	...
	*/
	...
	/**
		...
		* In case of network device the "host" is an IPv4 address or fully qualified domain name (FQDN), "serial" is the device serial number in hexadecimal system.
		...
		* Имя устройства имеет вид "xi-com:port" или "xi-net://host/serial" или "xi-emu:///file".
	...
	/**
		...
		* This library bundle contains a "keyfile.sqlite" file, which may be used as a default key.
		* \endenglish
		* \russian
		...
		* В комплекте библиотеки находится файл "keyfile.sqlite" который можно использовать как ключ по умолчанию.
		* \endrussian
	 */
